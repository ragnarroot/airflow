...
with DAG(
    dag_id='run_counterparty_check',
    default_args=default_args,
    start_date=datetime(2023, 1, 1),
    schedule="0 10 * * *",
    catchup=False,
    description='Check counnterparty limits',
) as dag:

    run_check_counterparty = DockerOperator(
        task_id='check_if_missing_counterparty',
        image='python-ubuntu',
        api_version='auto',
        auto_remove="success",
        command='/app/scripts/checks/check_counterparty.py',
        network_mode='bridge',
        mounts=[
    ...
    run_check_counterparty_ratios = DockerOperator(
        task_id='check_counterparty_ratios',
        image='python-ubuntu',
        api_version='auto',
        auto_remove="success",
        command='/app/scripts/checks/check_counterparty_ratios.py',
        network_mode='bridge',
        mounts=[
            Mount(source='/home/sl-ragnar/airflow/python_scripts', target='/app/scripts', type='bind')
        ], mount_tmp_dir=False,
    ...
    run_check_counterparty >> run_check_counterparty_ratios
